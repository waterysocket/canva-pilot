...
export class ExecutionEngine {
  ...
  public async executePlan(task: Task, plan: Plan, providerId: string): Promise<boolean> {
  ...
  private async executeStep(step: PlanStep): Promise<void> {
    ...
    switch (step.action) {
      case 'click':
        if (step.target) await this.automationEngine.click(step.target);
        break;
      case 'type':
        if (step.target && step.value) await this.automationEngine.type(step.target, step.value);
        break;
      case 'navigate':
        if (step.target) await this.automationEngine.navigate(step.target);
        break;
      case 'observe':
      case 'verify':
